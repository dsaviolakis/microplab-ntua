...
float process_temp_data() {
	...
	if (temp_data == 0x8000) {
		...
		int integer_part = OFFSET; //Add offset to simulate body temp
...
char* get_status(int status_num) {//Decode status from int to string 
	char* status;
...
int main() {
	...
	int status_num = 0;
	while(1) {
		pressure = process_pressure_data(); //Get pressure as float
		temperature = process_temp_data(); //Get temp as float
		status_num = update_status(pressure, temperature, status_num); //Renew status

		lcd_clear_display(); //Print data on screen
		print_string("TMP:");
		print_float(temperature);
		print_string(" PR:");
		print_float(pressure);

		char *status = get_status(status_num); //Convert status to string
		
		for(int i=0; i<24; i++) {//Go to next line
			lcd_data(' ');
		}

		print_string(status); //Print status
